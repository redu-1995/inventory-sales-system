...
import api from '../../services/api'; // Used to fetch products/suppliers list for dropdowns

export const PurchaseOrderModal = ({ isOpen, onClose, onOrderCreated }) => {
    const inventoryHook = useInventory();
  console.log("DEBUG: What did useInventory() return?", inventoryHook);
  
  // 3. Log the extracted function
  console.log("DEBUG: Is createPurchaseRequest extracted?", inventoryHook?.createPurchaseRequest);
  const { createPurchaseRequest, loading, error, success } = useInventory();

  console.log("DEBUG: What is the imported useInventory?", useInventory);
  // Form State
      ...
      const fetchData = async () => {
        try {
          const [suppliersRes, productsRes] = await Promise.all([
            api.get('products/suppliers/'), // Adjust endpoint mapping if your backend routing differs
  ...
  // Reset local form states on success or close
  useEffect(() => {
    if (success) {
      setSupplier('');
  ...
  const handleSubmit = async (e) => {
    ...
    // Assemble the clean formatted payload
    const payload = {
      supplier,
    ...
    if (result && onOrderCreated) {
      onOrderCreated(result);
    }
  };

 return (
  <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-900/10 backdrop-blur-sm p-4">
    <div className="relative w-full max-w-3xl rounded-xl bg-white shadow-[0_20px_50px_rgba(0,0,0,0.15),_0_0_1px_rgba(0,0,0,0.1)] transition-all">
      ...
      <form onSubmit={handleSubmit} className="p-6">
        ...
        {/* Error Banner */}
        {error && (
          <div className="mb-5 flex items-start gap-2.5 rounded-lg bg-red-50 p-3.5 text-sm text-red-700">
          ...
          >
            ...
            {suppliersList.map((sup) => (
              <option key={sup.id} value={sup.id}>
                {sup.name} {sup.contact_person ? `(${sup.contact_person})` : ''}
              </option>
            ))}
          </select>
        </div>
