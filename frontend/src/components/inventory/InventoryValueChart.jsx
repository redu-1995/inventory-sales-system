...
export default function InventoryValueChart({ trend }) {
  ...
  // Formatting utility for Y-axis Birr displays (e.g., 50k Br, 1.2M Br)
  const formatYAxis = (value) => {
    if (value >= 1_000_000) return `${(value / 1_000_000).toFixed(1)}M Br`;
    if (value >= 1_000) return `${(value / 1_000).toFixed(0)}k Br`;
    return `${value} Br`;
  };
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm h-[380px] flex flex-col">
      <div className="mb-4">
        ...
        <p className="text-xs text-slate-500">Rolling total inventory cost valuation (ETB) over the last 7 days</p>
      </div>
